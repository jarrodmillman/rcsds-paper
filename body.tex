...
Our experience is that this assumption is incorrect.  It is rare for students
to go on to learn these foundations. They continue as they were when they
leave the course, as inefficient practitioners, and unable to reason about
their analysis.
...
The GLM is a widely-used term in neuroimaging, to refer
...
In contrast to the traditional approach, we put our own emphasis on:
...
The Berkeley course catalog had the following description of our course:
...
investigating a real scientific question, that they had chosen themselves.
...
Its strength in science rests on a stack of popular scientific libraries
including as NumPy (computing with arrays), SciPy (scientific algorithms
including optimization and image processing), Matplotlib (2D plotting),
Scikit-Learn (machine learning), and Pandas (data science).  The Nibabel
library \footnote{\url{http://nipy.org/nibabel}\label{nibabel}} loads file in
standard brain image formats into memory as arrays for manipulations and
...
the decisions that the team makes, and a justification for each accepted set
of changes.
...
code from view.
This can make it easier to see the high-level structure of your analysis.
...
Testing is an essential discipline to give some assurance that the code
continues to work as expected, to demonstrate that this is the case to other
users of the code, and to document how the code is expected to work.
...
thoroughly.  There are tools that can measure how much of the code has been
covered by the tests -- this is ``test coverage''. We expected students to
keep test coverage high, typically above 80\%.
...
For example a Makefile could define a label \textt{get_data}, thus:
...
\fixme{Slides were written in Markdown.
...
to generate PDFs that could be used as presentation slides.
Report was written in \LaTeX.
...
\fixme{Shouldn't these two paras above be in the material and methods?}
