...
Unlike the above example, Makefile rules often involve several commands.
The target of a rule is often a filename (e.g., \texttt{progress.pdf}), but can
also be an arbitrary name for some commands that you may want to envoke.
The dependencies of a rule can be filenames (e.g., \texttt{progress.pdf}), but
may also be other rules in the Makefile.
\fixme{This ability to chain ... complex ... reproducible workflows.}
...
We assigned two homeworks during the first half of the semester.
...
\fixme{See~\ref{fig:progress}
Progress presentation and draft report (week~12) ...
...
They had to commit the final PDF of their reports as well as provide
instructions for generating the PDF from the source \LaTeX files.
We also required that each group prepare a slide presentation discussing where
things were and there plan for finalizing their project two weeks before they
were due.
...
% do we need a section on is a substantial project necessary?
% or is it more a theme than spans multiple of the discussion subsections?

% what about teaching staff?
% we had 4 instructors representing expertise in almost every aspect of
% neuroimaging research  (neuroscience, neuroimaging, psychology, statistics, computing, physics, ...)
% it may be hard to duplicate. would a smaller, less diverse teaching staff be
% able to handle so many diverse, open-ended projects, which are graded
% in such detail?
