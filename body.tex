...
We define an analysis as computationally reproducible when someone other than
the original author of an analysis can take the author's data, code and
instructions, and reproduce their analysis.

Very few neuroimaging analyses are reproducible, even between researchers in a
single lab.  The process of analysis is typically ad-hoc and informal; a
analysis is often the fruit of a considerable amount of trial and error, based
in part on scripts and data structures that the author inherited from past and
current members of the lab.   We are describing typical analysis *process* in
order to argue that this *process* is a) confusing, leading to unclear
hypotheses and conclusions, b) error prone, leading to incorrect conclusions
and greater confusion and c) an impossible foundation on which to build
reproducible analyses.

The trifecta of confusion, error and lack of reproducibility are ubiquitous
problems of computation that programmers have been fighting since "programmer"
became a job description. There are widely accepted tools and processes to
...
effectively teach *both* the tools for efficient and reproducible work *and*
...
code they had written themselves.  We required the analysis to be entirely
replicable; the graders had to be able to run a simple set of commands to
fetch the data, run the analysis, and generate the final report, including the
figures.  As we will report in the results, our students were remarkably
successful in achieving this goal.  We will argue that this approach to
training in neuroimaging is likely to be far more fruitful than troditional
...
This course was specifically designed to be as ``practical'' and ``hands on''
as possible.
...
Bootcamp as introduction and supplement to a course like ours.
...
%\jb{ \emph{The reproducibility crisis in science (and especially life
%sciences): a strong rationale for changing how teaching is done.}
%% * JB: Rationale 1:  Reproducibility in life/science and neuroimaging
\jb{Since the seminal papers of J. Ioannidis \citep{ioannidis_why_2005}, the
issue of reproducibility or replicability in life science has been debated in
the literature and attracted the attention of the research community, the
funders and the journals editors (see for instance amongst many
\citep{collins_nih_2014, allison_reproducibility, horton_offline:_2016}, and in
brain imaging \citep{boekel_purely_2013}).
Since researchers seem to produce a large number of irreproducible results, it
is important to consider the causes of this irreproducibility and what can be
done to curb the issue.
One direct answer is to work to provide the future researchers with the tools
and good practices that foster scientific reproducibility
\citep{janz_bringing_2015}.}
...
%Homework    20%
%Project 55%
% everything but project was graded individually

\item Quizzes have several advantages: they can be corrected automatically, are
quick to perform.
\fixme{They can be challenging to construct in some situations, and are not always
best fitted for evaluating the most important pedagogical components.
In our case, we emphasized the collaborative and reproducibility aspects and
quizzes can not evaluate the overall success in making a reproducible project.}

\item % KJM - this should be homework (not exercises)
Exercises typically would be designed to assess competency on one (or
possibly several) key concepts. % they were designed to help the students master the material
...
% they were hard to design, but they were easy to mark as we used
% scripts to grade them.
...
% KJM - We didn't give students individual grades for the project, so I am not
% sure we should address this point here.
%First, evaluation is individual, and while Git should permit to assess each
%individual contribution, evaluation of each student contribution to the project
%remains a fairly difficult task.

% KJM-I didn't have a separate participation part of the grade, so I am not
% sure we should address this point here. 
%\item Participation and laboratory sessions.
